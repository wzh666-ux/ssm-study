                    ...
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="gsno" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
                            <label class="layui-form-label">仓库名称</label>
                            <div class="layui-input-inline">
                                <input type="text" name="gsname" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
                            <label class="layui-form-label">仓库类别</label>
                            <div class="layui-input-inline">
                                <select name="cyno" id="cyno" lay-search="">
                                    <option value=""></option>
                                </select>
                            </div>
                        </div>
    ...
    layui.use(['form', 'table'], function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                $.each(result,function (index, item) {
                    $('#cyno').append(new Option(item.cyname,item.cyno));  //循环将科类信息放入下拉选择框
                });
                form.render(); //刷新表单
            }
        });
        table.render({    //刷新表格数据
            ...
            cols: [[
                ...
                {field: 'gsno',  title: '仓库编号', sort: true},
                {field: 'gsname',  title: '仓库名称', sort: true},
                {field: 'cyname',  title: '仓库类别', sort: true},
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {//toolbar 表头里按钮的动作事件
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    title: '<h1 style="text-align: center">添加仓库信息</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    area: ['50%', '75%'],
                    content: 'genusForm.jsp',
                });
        ...
        table.on('tool(currentTableFilter)', function (obj) { // tool 表格里的操作按钮的动作事件
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    title: '<h1 style="text-align: center">编辑仓库信息</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    ...
                    content: 'genusForm.jsp',
                    success: function (layero, index) {  //页面打开成功后执行的动作,回显数据到编辑页面
                        var body = layer.getChildFrame('body',index);
                        body.find("#gsno").val(data.gsno);
                        body.find("#gsname").val(data.gsname);
                        body.find("#remark").val(data.remark);
                        body.find("#id").val(data.id);
                        body.find("#cyno").val(data.cyno);
                        console.log(data)
                        //body.find('#cyno option[value=data.cyno]').attr("selected", true);
