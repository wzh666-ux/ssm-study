                ...
                <form class="layui-form layui-form-pane" action="">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label">维护设备</label>
    ...
    layui.use(['form', 'table'], function () {
        ...
        table.render({    //刷新表格数据
            ...
            cols: [[
                ...
                {field: 'name',  title: '维护设备名称', sort: true},
                {field: 'time',  title: '预计维护时间', sort: true},
                {field: 'weiname',  title: '维护人员名称', sort: true},
                {field: 'weiphone',  title: '人员联系方式', sort: true},
        ...
        $.ajax({
            ...
            success: function (result) {
                $.each(result,function (index, item) {
                    $('#name').append(new Option(item.name,item.name));  //循环将属类信息放入下拉选择框
                });
                form.render(); //刷新表单
            }
        });
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {//toolbar 表头里按钮的动作事件
            if (obj.event === 'add') {  // 监听添加操作
                ...
                var index = layer.open({
                    title: '<h1 style="text-align: center">添加维护设备</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    area: ['50%', '75%'],
                    content: 'deviceWeiForm.jsp',
                });
        ...
        table.on('tool(currentTableFilter)', function (obj) { // tool 表格里的操作按钮的动作事件
            ...
            if (obj.event === 'edit') {
                json = JSON.stringify(obj.data);
                var index = layer.open({
                    title: '<h1 style="text-align: center">编辑维护设备</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    ...
                    success: function (o, index) {  //页面打开成功后执行的动作,回显数据到编辑页面
                        ...
                        body.find("#weiname").val(data.weiname);
                        body.find("#weiphone").val(data.weiphone);
