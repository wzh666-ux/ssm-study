                ...
                <form class="layui-form layui-form-pane" action="">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label">材料生产地址</label>
...
<script>
    ...
    layui.use(['form', 'table'], function () {
        ...
        table.render({    //刷新表格数据
            elem: '#currentTableId',
            url: '/logisticsgotos/list',  //表格的数据来源
            toolbar: '#toolbarDemo',
            ...
            cols: [[
                ...
                {field: 'name',  title: '材料生产地址', sort: true},
                {field: 'carname',  title: '所属仓库', sort: true},
                {field: 'peiname',  title: '管理人', sort: true},
                {field: 'peiphone',  title: '管理人联系方式', sort: true},
                {field: 'time',  title: '入库时间', sort: true},
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {//toolbar 表头里按钮的动作事件
            if (obj.event === 'add') {  // 监听添加操作
                ...
                var index = layer.open({
                    title: '<h1 style="text-align: center">添加原材料</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    area: ['50%', '75%'],
                    content: 'logisticsGotosForm.jsp',
                });
                ...
                layer.confirm('是否确定删除选中行？', function (index) {  //confirm 确定取消的对话框
                    ...
                    $.ajax({
                        url: "/logisticsgotos/deleteBatch",
        ...
        table.on('tool(currentTableFilter)', function (obj) { // tool 表格里的操作按钮的动作事件
            ...
            if (obj.event === 'edit') {
                json = JSON.stringify(obj.data);
                var index = layer.open({
                    title: '<h1 style="text-align: center">编辑原材料</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    ...
                    success: function (o, index) {  //页面打开成功后执行的动作,回显数据到编辑页面
                        ...
                        body.find("#carname").val(data.carname);
                        body.find("#peiname").val(data.peiname);
                        body.find("#peiphone").val(data.peiphone);
