...
<div class="layui-form layuimini-form">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="name" id="name" lay-verify="required" lay-reqtext="不能为空" placeholder="请输入车间名称"
                   class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label required">车间类型</label>
        <div class="layui-input-block">
    ...
    <div class="layui-form-item">
        <label class="layui-form-label required">原材料</label>
        <div class="layui-input-block">
            <select name="gotos" id="gotos" lay-search="">
                <option value=""></option>
            </select>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        <label class="layui-form-label required">管理人电话</label>
        <div class="layui-input-block">
            <input type="text" name="userphone" id="userphone" lay-verify="required" lay-reqtext="不能为空" placeholder="请输入管理人电话"
                   class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
...
<script>
    layui.use(['form'], function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                $.each(result,function (index, item) {
                    $('#gotos').append(new Option(item.name,item.name));  //循环将属类信息放入下拉选择框
                });
                form.render(); //刷新表单
            }
        });
        ...
        form.on('submit(saveBtn)', function (data) {
            ...
            if (data.field.id == null || data.field.id == "") {
                url="/logistics/save"  //如果id号为空，表明是添加动作
            }else{
                url="/logistics/update"//如果id号有值，表明是修改动作
            }
            //发送修改信息到后台
            $.ajax({
                url: url,
