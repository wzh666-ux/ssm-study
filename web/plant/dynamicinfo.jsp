                ...
                <form class="layui-form layui-form-pane" action="">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label">科类编号</label>
                            <div class="layui-input-inline">
                                <input type="text" name="cyno" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
                            <label class="layui-form-label">科类名称</label>
                            <div class="layui-input-inline">
                                <input type="text" name="cyname" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
    ...
    layui.use(['form', 'table'], function () {
        ...
        table.render({    //刷新表格数据
            ...
            cols: [[
                ...
                {field: 'basicstate',  title: '基本情况', sort: true},
                {field: 'isill',  title: '是否生病', sort: true},
                {field: 'checktime',  title: '检查时间'},
                {field: 'checker',  title: '检查人', sort: true},
                {field: 'ismove',  title: '是否转移', sort: true},
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {//toolbar 表头里按钮的动作事件
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    title: '<h1 style="text-align: center">添加植物动态信息</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    area: ['58%', '75%'],
                    content: 'dynamicinfoForm.jsp',
                });
        ...
        table.on('tool(currentTableFilter)', function (obj) { // tool 表格里的操作按钮的动作事件
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    title: '<h1 style="text-align: center">编辑植物动态信息</h1>',
                    type: 2,
                    shade: 0.2,
                    maxmin:true,
                    shadeClose: true,
                    area: ['1000px', '75%'],  //打开弹出框时，页面的宽高比例
                    content: 'dynamicinfoForm.jsp',
                    success: function (o, index) {  //页面打开成功后执行的动作,回显数据到编辑页面
                        var body = layer.getChildFrame('body',index);
                        body.find("#plantno").val(data.plantno);
                        body.find("#addr").val(data.addr);
                        body.find("#remark").val(data.remark);
                        body.find("#id").val(data.id);
                        body.find("#basicstate").val(data.basicstate);
                        //单选框的回显
                        var iframeWin = o.find('iframe')[0].contentWindow;// 新iframe窗口的对象
                        iframeWin.layui.form.render('select');// 重新渲染checkbox,select同理
                        iframeWin.layui.$('input[name="isill"][value=' + data.isill + ']').prop("checked", true);
                        iframeWin.layui.$('input[name="ismove"][value=' + data.ismove + ']').prop("checked", true);
                        iframeWin.layui.form.render('radio');// 重新渲染radio
                        iframeWin.showBMap(data.longitude, data.latitude, data.addr, ''); //调用显示地图回显
                    }
                });
                $(window).on("resize", function () {
                    layer.full(index);
