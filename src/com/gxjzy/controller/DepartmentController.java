...
import com.gxjzy.dao.DepartmentMapper;
...
public class DepartmentController {
    ...
    public Map<String,Object> listView(int page, int limit, String searchParams){   //查询科类列表并返回
        ...
        //调用分页查询
        List<Department> list = departmentMapper.getListView((page-1)*limit,limit,where);
        //List<PlCategory> list = plCategoryMapper.list();
        //获取数据的总条数
        int count = departmentMapper.getListCount(where);
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "成功查询到数据！");
    ...
    public Map delete(String id) {
        int result = departmentMapper.delete(id);  //调用删除接口
        Map map = new HashMap();
        if (result > 0) {
    ...
    @ResponseBody
    public Map deleteBatch(@RequestBody List<Department> departments) {
        int result = departmentMapper.deleteBatch(departments);  //调用批量删除接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "批量删除成功！");
    ...
    public Map update(@RequestBody Department department) {
        int result = departmentMapper.update(department);  //调用修改接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "修改成功！");
    ...
    @RequestMapping("/save")
    @ResponseBody
    public Map save(@RequestBody Department department) {
        int result = departmentMapper.save(department);//调用保存接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "保存成功！");
