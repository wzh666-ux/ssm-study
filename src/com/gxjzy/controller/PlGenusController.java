...
import com.gxjzy.dao.PlGenusMapper;
...
public class PlGenusController {

    @Autowired
    PlGenusMapper plGenusMapper;

    //查询所有科类给下拉选择使用
    @RequestMapping("/listAll")
    @ResponseBody
    public List<PlGenus> listAll(){
        List<PlGenus> list = plGenusMapper.list();
        return list;
    }
    ...
    public Map<String,Object> listView(int page,int limit,String searchParams){   //查询科类列表并返回
        ...
        //调用分页查询
        List<PlGenus> list = plGenusMapper.getListView((page-1)*limit,limit,where);
        //List<PlGenus> list = plGenusMapper.list();
        //获取数据的总条数
        int count = plGenusMapper.getListCount(where);
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "成功查询到数据！");
    ...
    public Map delete(String id) {
        int result = plGenusMapper.delete(id);  //调用删除接口
        Map map = new HashMap();
        if (result > 0) {
    ...
    @ResponseBody
    public Map deleteBatch(@RequestBody List<PlGenus> plGenuses) {
        int result = plGenusMapper.deleteBatch(plGenuses);  //调用批量删除接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "批量删除成功！");
    ...
    @ResponseBody
    public Map update(@RequestBody PlGenus plGenus) {
        int result = plGenusMapper.update(plGenus);  //调用修改接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "修改成功！");
    ...
    @RequestMapping("/save")
    @ResponseBody
    public Map save(@RequestBody PlGenus plGenus) {
        int result = plGenusMapper.save(plGenus);//调用保存接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "保存成功！");
