...
public class UserinfoController {
    ...
    public List<Role> roleList(){
        List<Role> list = plUserinfoMapper.role();
        return list;
    }
    ...
    @ResponseBody
    public List<PlUserinfo> listAll(){
        List<PlUserinfo> list = plUserinfoMapper.list();
        return list;
    }
    ...
    public Map<String,Object> listView(int page, int limit, String searchParams){   //查询科类列表并返回
        ...
        //调用分页查询
        List<PlUserinfo> list = plUserinfoMapper.getListView((page-1)*limit,limit,where);
        //List<PlCategory> list = plCategoryMapper.list();
        //获取数据的总条数
        int count = plUserinfoMapper.getListCount(where);
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "成功查询到数据！");
    ...
    public Map delete(String id) {
        int result = plUserinfoMapper.delete(id);  //调用删除接口
        Map map = new HashMap();
        if (result > 0) {
    ...
    @ResponseBody
    public Map deleteBatch(@RequestBody List<PlUserinfo> plUserinfos) {
        int result = plUserinfoMapper.deleteBatch(plUserinfos);  //调用批量删除接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "批量删除成功！");
    ...
    @ResponseBody
    public Map update(@RequestBody PlUserinfo plUserinfo) {
        int result = plUserinfoMapper.update(plUserinfo);  //调用修改接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "修改成功！");
    ...
    @ResponseBody
    public Map insert(@RequestBody PlUserinfo plUserinfo) {
        int result = plUserinfoMapper.insert(plUserinfo);//调用保存接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "保存成功！");
