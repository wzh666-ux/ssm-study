...
import com.gxjzy.dao.DeviceMapper;
...
public class DeviceController {
    ...
    public List<Device> listAll(){
        List<Device> list = deviceMapper.list();
        return list;
    }
    ...
    public Map<String,Object> listView(int page, int limit, String searchParams){   //查询科类列表并返回
        ...
        //调用分页查询
        List<Device> list = deviceMapper.getListView((page-1)*limit,limit,where);
        //List<PlCategory> list = plCategoryMapper.list();
        //获取数据的总条数
        int count = deviceMapper.getListCount(where);
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "成功查询到数据！");
    ...
    public Map delete(String id) {
        int result = deviceMapper.delete(id);  //调用删除接口
        Map map = new HashMap();
        if (result > 0) {
    ...
    @ResponseBody
    public Map deleteBatch(@RequestBody List<Device> devices) {
        int result = deviceMapper.deleteBatch(devices);  //调用批量删除接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "批量删除成功！");
    ...
    @ResponseBody
    public Map update(@RequestBody Device device) {
        int result = deviceMapper.update(device);  //调用修改接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "修改成功！");
    ...
    @RequestMapping("/save")
    @ResponseBody
    public Map save(@RequestBody Device device) {
        int result = deviceMapper.save(device);//调用保存接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "保存成功！");
