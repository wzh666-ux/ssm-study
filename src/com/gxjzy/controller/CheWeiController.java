...
import com.gxjzy.dao.CheWeiMapper;
...
public class CheWeiController {
    ...
    @ResponseBody
    public List<CheWei> listAll(){
        List<CheWei> list = cheWeiMapper.list();
        return list;
    }
    ...
    public Map<String,Object> listView(int page, int limit, String searchParams){   //查询科类列表并返回
        ...
        //调用分页查询
        List<CheWei> list = cheWeiMapper.getListView((page-1)*limit,limit,where);
        //List<PlCategory> list = plCategoryMapper.list();
        //获取数据的总条数
        int count = cheWeiMapper.getListCount(where);
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "成功查询到数据！");
    ...
    public Map delete(String id) {
        int result = cheWeiMapper.delete(id);  //调用删除接口
        Map map = new HashMap();
        if (result > 0) {
    ...
    @ResponseBody
    public Map deleteBatch(@RequestBody List<CheWei> cheWeis) {
        int result = cheWeiMapper.deleteBatch(cheWeis);  //调用批量删除接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "批量删除成功！");
    ...
    @ResponseBody
    public Map update(@RequestBody CheWei cheWei) {
        int result = cheWeiMapper.update(cheWei);  //调用修改接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "修改成功！");
    ...
    @RequestMapping("/save")
    @ResponseBody
    public Map save(@RequestBody CheWei cheWei) {
        int result = cheWeiMapper.save(cheWei);//调用保存接口
        Map map = new HashMap();
        if (result > 0) {
            map.put("status", 1); //成功状态
            map.put("msg", "保存成功！");
